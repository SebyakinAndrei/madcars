...
class Game(object):
    ...
    def __init__(self, clients, games_list, extended_save=True, should_dump=False, is_rl_env=False, env_map=None, env_car=None):
        self.game_complete = False
        self.extended_save = extended_save
        self.should_dump = should_dump
        self.is_rl_env = is_rl_env
        self.env_map, self.env_car = env_map, env_car

        self.max_match_count = math.ceil(len(games_list)/2)
        self.all_players = [Player(index + 1, client, self.max_match_count, should_dump=should_dump) for index, client in enumerate(clients)]
    ...
    def next_match(self):
        if self.should_dump and self.current_match is not None:
            path = 'dumps/{}_{}'.format(self.env_map, self.env_car)
            if not isdir(path):
                mkdir(path)
            with open('dumps/{}_{}/dump_{}.dat'.format(self.env_map, self.env_car, random.randint(0, 10**8)), 'w') as f_out:
                for player in self.all_players:
                    #print('dead_players: ', list(self.current_match.dead_players))
                    if len(player.dump) > 0:
                        print(ujson.dumps({'id': player.id, 'dump': player.dump, 'win': player not in self.current_match.dead_players}), file=f_out, flush=True)
                    player.dump.clear()
    ...
    @asyncio.coroutine
    def game_loop(self, nolimit=False):
        if not nolimit:
            from tqdm import trange
            for i in trange(MAX_TICK_COUNT):
                #if i % 2000 == 0:
                #    print('tick {}'.format(i))
